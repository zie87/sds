...
struct rope_tests : ut::testcase<> 
{

  static void run()
  {
      UNITTEST_CLASS(rope_tests);
      UNITTEST_RUN(simple_test);
      // UNITTEST_RUN(fill_test);
  }

  rope_tests() {} // executed before each test and before set_up()
  ~rope_tests() {} // executed after each test and after tear_down()
...
REGISTER(rope_tests) // this registers the test class